...
- `tasks.json`: defines custom build tasks.
  ...
  - evccg: evccg: Generate JQ - see [further below for details](#folder-scriptsgenerate-jq)

<br>

## Folder `/ciq-store`

Contains images used to present the app in the Connect IQ Store. The source files for the screenshots are located in [`/screenshots`](#folder-screenshots), while the versions in this folder are resized and optimized to meet the Connect IQ Store image size requirements.
...
Contains screenshots used both for the Connect IQ Store and the documentation.

<br>

## Folder `/scripts`

Contains scripts used to build the app.
...
The app uses a server-side JQ filter to reduce the JSON returned by the evcc REST API to only the fields required by the app, as Garmin imposes relatively strict limits on incoming JSON payload sizes. In addition to filtering, the JQ logic also aggregates data for display purposes, such as the grid price forecast.

The JQ filter is passed from the Monkey C code as part of the HTTP request to evcc and therefore needs to be available in a minified form without comments or line breaks.

For easier editing, the source JQ is maintained in the formatted file `/source/jq/state.jq`. The scripts in this folder use it to generate `/source/jq/minified.mc`, a piece of Monkey C code that stores the minified JQ in a global variable.

`generate-jq.bat` can be executed from the command line and runs `generate-jq.js`, a WSH JavaScript script that generates the minified version.
...
- `/source/jq`: The app uses a server-side JQ filter to reduce the JSON returned by the evcc REST API to only the fields required by the app, as Garmin imposes relatively strict limits on incoming JSON payload sizes. In addition to filtering, the JQ logic also aggregates data for display purposes, such as the grid price forecast.

  - `state.jq`: Contains the formatted JQ source and should be modified when changes are needed.
  - `minified.mc`: Contains a minified version of the JQ stored in a Monkey C global variable. This file is generated by script; see [here](#folder-scriptsgenerate-jq) for details.
...
| fenix9pro47mm        | 6.0.3         |
| fenix9pro51mm        | 6.0.3         |
| fenix9prosolar47mm   | 6.0.3         |
| fenix9prosolar51mm   | 6.0.3         |
| fr955                | 5.1.0         |
| fr165                | 5.1.0         |
| fr165m               | 5.1.0         |
| fr265                | 5.1.0         |
