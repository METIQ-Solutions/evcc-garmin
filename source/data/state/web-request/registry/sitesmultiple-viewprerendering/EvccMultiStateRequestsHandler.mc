...
public class EvccMultiWebRequestsHandler {
    private var _stateRequests as Array<WebRequest>;
    ...
    public function initialize( stateRequests as Array<WebRequest>, activeSite as Number ) {
        // HelperBase.debug( "EvccMultiWebRequestsHandler: initializing with " + stateRequests.size() + " state requests" );
        _stateRequests = stateRequests;
        _activeSite = activeSite;
        _initialActiveSite = activeSite;

        // HelperBase.debug( "EvccMultiWebRequestsHandler: initiating state request for site " + activeSite );
        ...
        if( _stateRequests.size() > 1 ) {
            // HelperBase.debug( "EvccMultiWebRequestsHandler: starting delayed initiation" );
