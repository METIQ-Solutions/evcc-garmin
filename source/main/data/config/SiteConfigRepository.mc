...
(:glance :background :exclForSitesOne) class SiteConfigRepository {
...
(:glance :background :exclForSitesMultiple) class SiteConfigRepository {
    static function getSiteCount() as Number { 
        return  ( Properties.getValue( 
                    Constants.PROPERTY_SITE_PREFIX + "0" + Constants.PROPERTY_SITE_URL_SUFFIX 
                ) as String ).equals( "" )
                ? 0
                : 1;
   }
