...
// This class draws the hint (indicator) that lower-level views are
// available and can be opened using the device's onSelect behavior.
//
// The following hint types are currently supported:
// - on round screens, an arc indicating the button to be pressed,
...
// use for a device, and excludes all other code.

// This version implements support for round screens.
(:exclForScreenRectangular) 
class SelectIndicator {
    // Draw function for arc
    // Angle can be either 27° or 30°
    private var SELECT_RADIUS_FACTOR as Float = 0.49; // factor applied to dc width to calculate the radius of the arc
    private var SELECT_LINE_WIDTH_FACTOR as Float = 0.01; // factor applied to dc width to calculate the width of the arc
    private var SELECT_LENGTH as Number = 18; // total length of the arc in degree
    public function draw( dc as Dc ) as Void {
        ...
        var selectCenterAngle = DeviceProperties.get().SELECT_INDICATOR_ANGLE;
        var from = selectCenterAngle - SELECT_LENGTH / 2;
        var to = selectCenterAngle + SELECT_LENGTH / 2;
...
class SelectIndicator {
    ...
    public function draw( dc as Dc ) as Void {
        ...
        // Calculate all parameters for the line
        var lineWidth = dc.getWidth() * SELECT_LINE_WIDTH_FACTOR;
        var lineLength = dc.getHeight() * SELECT_LENGTH_FACTOR;
        var x = dc.getWidth() - lineWidth/2;

        // The vertical position is expressed as a value from 0 to 100
        // and is converted to a percentage by multiplying by 0.01.
        var yStart = 
            dc.getHeight() * 0.01 * DeviceProperties.get().SELECT_INDICATOR_VERTICAL_POSITION
            - lineLength/2;
