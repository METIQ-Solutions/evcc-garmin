...
//   - or if all is well obtaining actual content from implementations of EvccSiteViewBase
...
class SiteContent {
    ...
    protected var _content as VerticalBlock?;
    
    // The alreadyHasRealContent flag is only needed for pre-rendering views
    private var _alreadyHasRealContent as Boolean = false;
    public function alreadyHasRealContent() as Boolean { return _alreadyHasRealContent; }
    private function setAlreadyHasRealContent() as Void { _alreadyHasRealContent = true; }
    ...
    protected function assembleInternal( calcDc as EvccDcInterface ) as VerticalBlock {
        ...
        var content = new VerticalBlock( { :dc => calcDc } as DbOptions );
        ...
        var font = WidgetResourceSet.FONT_MEDIUM; // We start with the largest font
    ...
    public function prepare() as Void {
        var ca = _view.getContentArea();
        ( _content as VerticalBlock ).prepareDraw( ca.x, ca.y );
    }
    ...
    public function draw( dc as Dc ) as Void {
        ...
        ( _content as VerticalBlock ).drawPrepared( dc );
    }
}