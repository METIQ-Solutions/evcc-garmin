...
class EvccWidgetLoadPointView extends EvccWidgetBaseSiteView {
    ...
    *
    * While EvccDetailViewManager already defines the number of pages,
    ...
    */
    ...
    private function addLoadpoint( 
        block as EvccVerticalBlock, 
        loadpoint as EvccLoadPoint, 
        isAnyLoadpointCharging as Boolean 
    ) as Void {
        var controllable = loadpoint.getControllable();
        if( controllable == null ) {
            throw new OperationNotAllowedException("EvccWidgetLoadPointView.addIntegratedDevice: loadpoint does not have controllable" );
        }

        var titleLine = new EvccHorizontalBlock( { :truncateSpacing => getContentArea().truncateSpacing } );
        ...
        if( controllable instanceof EvccConnectedVehicle ) {
            titleLine.addText( ": " + EvccHelperUI.formatSoc( controllable.getSoc() ) );
        } else if( controllable instanceof EvccHeater ) {
            titleLine.addText( ": " + EvccHelperWidget.formatTemp( controllable.getTemperature() ) );
        } 
        
        var stateLine = new EvccHorizontalBlock( { :relativeFont => 3 } );

        if( isAnyLoadpointCharging ) {
            stateLine.addIcon( 
                EvccIconBlock.ICON_ACTIVE_PHASES, 
                { :charging => true, 
        ...
        stateLine.addText( 
            EvccHelperWidget.formatPower( loadpoint.getChargePowerRounded() )
            + " (" + EvccHelperWidget.formatMode( loadpoint ) + ")"
        );

        block.addBlock( stateLine );

        if( controllable instanceof EvccConnectedVehicle && loadpoint.getChargeRemainingDuration() > 0 ) {
            var timeLine = new EvccHorizontalBlock( { :relativeFont => 3 } );
            timeLine.addIcon( EvccIconBlock.ICON_DURATION, {} as DbOptions );
            timeLine.addText( " " + EvccHelperWidget.formatDuration( loadpoint.getChargeRemainingDuration() ) );
            block.addBlock( timeLine );
        }
    }

    // Show the category icon as page title
    public function getPageIcon() as EvccIconBlock? {
